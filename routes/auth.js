...
router.post('/register', RegisterValidator, handleResultValidator, async function (req, res, next) {
    try {
        // Prevent duplicate user/email registrations
        let existedByUsername = await userController.FindByUsername(req.body.username);
        if (existedByUsername) {
            return res.status(400).send({ message: "Username đã tồn tại" });
        }

        let existedByEmail = await userController.FindByEmail(req.body.email);
        if (existedByEmail) {
